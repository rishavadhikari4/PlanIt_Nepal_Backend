...
const { sendVerificationOTP } = require('../utils/emailHelper');
...
exports.sendVerificationMail = async (req, res) => {
  ...
  try {
    ...
    try {
      const jobId = queueVerificationOTP(user.email, user.name, OTP_CODE);
      console.log(`📧 Verification email queued (Job ID: ${jobId}) for:`, user.email);
